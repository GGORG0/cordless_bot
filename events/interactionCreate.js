			...
			if (interaction.customId.startsWith('roleselect')) {
				...
				if (interaction.values[0] !== '_clear') {
					...
					await interaction.member.roles.add(role)
						.then(() =>
							interaction.editReply({ content: `You have been given the role **${interaction.values[0].replace('_', ' ')}**!`, ephemeral: true }));
				}
				else {
					await interaction.editReply({ content: 'You have been removed from all selectable roles!', ephemeral: true });
				}
			}
		}
	},
