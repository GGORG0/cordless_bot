...
module.exports = {
	...
	async execute(interaction) {
		...
		if (interaction.options.getSubcommand() === 'add') {
			...
			await db.set('blocked', blocked.concat([interaction.options.getUser('user').id]));
			...
			await db.set('blocked', blocked.filter(user => user !== interaction.options.getUser('user').id));
