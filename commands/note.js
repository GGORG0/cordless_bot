	...
	async execute(interaction) {
		...
		if (interaction.options.getSubcommand() === 'create') {
			...
			interaction.channel.awaitMessages({
				...
				await notes.set(`${interaction.guild.id}.${interaction.options.getString('name')}`, input.first().content);
				await notes.set(`${interaction.guild.id}`, gnotes.concat([interaction.options.getString('name')]));
			...
			if (notes.get(`${interaction.guild.id}.${interaction.options.getString('name')}`) === undefined) {
				interaction.reply('Note does not exist.');
				return;
			}

			await notes.delete(`${interaction.guild.id}.${interaction.options.getString('name')}`);
			await notes.set(`${interaction.guild.id}`, gnotes.filter(n => n !== interaction.options.getString('name')));
			...
			const note = await notes.get(`${interaction.guild.id}.${interaction.options.getString('name')}`);
